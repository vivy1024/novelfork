...
- 活跃 spec 在 `.kiro/specs/` 根目录；当前主线为 `backend-contract-v1` → `frontend-refoundation-v1`，`backend-core-refactor-v1` 作为后续整理阶段
- 每个 spec 必须通过 typecheck + test 才能标记完成

## AI 输出原则

- AI 结果只进候选区/草稿区，用户确认后才影响正式正文
- Studio 已交付口径是 session-first 工作台；当前重建主线已完成 Backend Contract 验收收口，Frontend Refoundation 执行中（10/12）：`/next` 使用 Agent Shell 路由壳，Conversation runtime、单栏 surface、模型/权限状态栏动作、Tool Result Renderer Registry、Workbench 资源树、canvas/viewer 与写作动作入口已接入，主入口已切断旧三栏 WorkspacePage 默认依赖；所有按钮、资源节点和写作动作必须来自 `packages/studio/src/app-next/backend-contract/` 的真实合同 client / adapter
