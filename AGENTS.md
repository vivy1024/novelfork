...
- Studio 已交付口径是 session-first 工作台；当前重建主线已完成 Backend Contract 验收收口，Frontend Refoundation 执行中（4/12）：`/next` 使用 Agent Shell 路由壳，Conversation runtime 与单栏 surface 已接入，所有按钮、资源节点和写作动作必须来自 `packages/studio/src/app-next/backend-contract/` 的真实合同 client / adapter
