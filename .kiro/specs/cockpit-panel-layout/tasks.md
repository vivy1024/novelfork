...
- [x] A4. BeatProgressBar 嵌入 StatCard 网格（进度条形态）
- [x] A5. DefaultCockpitView 重写上半区为 StatCard 2×3 网格（fetch overview-stats）
- [x] A6. StatCard 点击联动（伏笔→展开 foreshadowing panel / 平台→展开 compliance / 弧线→展开 arcs）

## Phase B：写作区面板集成（扩展 writing-scene-spec-ui）

- [x] B1. 编辑器底部可展开工具栏组件 ChapterToolbar（ChapterHealthCard + AiTasteReport tab 切换）
...
- [x] C1. PresetsPanel 接入设定区 rules 分类下子视图
- [x] C2. TemplateMarketPanel 注册为 Routines 页 plugin section
- [x] C3. ConversationResourcePanel 接入 agent 对话面板侧栏
- [x] C4. PresetSuggestionCard：建书完成后弹推荐预设提示

## Phase D：跨区联动 + 验证

- [x] D1. URL 路由参数联动设计（?area=overview&panel=compliance）
- [ ] D2. StatCard 点击→ExpandablePanel 展开（A6 的路由版）
- [x] D3. 设定区伏笔"目标章节"→跳写作区打开该章
