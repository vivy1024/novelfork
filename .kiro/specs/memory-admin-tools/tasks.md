...
- [ ] 1. ENABLER: 建立 `memory-admin-handlers` 基础结构
...
- [ ] 2. FEATURE: 实现只读条目枚举 `memory.list`
...
- [ ] 3. FEATURE: 实现单条读取 `memory.read_entry`
...
- [ ] 8. FEATURE: 实现受控更新 `memory.update`
  ...
  - event 允许更新 chapterNumber/eventType/subject/predicate/object/evidenceText/confidence/source/status/riskLevel/appliedAt。
...
- [ ] 9. FEATURE: 实现受控硬删除 `memory.delete`
...
- [ ] 10. FEATURE: 实现批量批准 `memory.bulk_approve`
...
- [ ] 11. FEATURE: 实现批量删除 `memory.bulk_delete`
...
- [ ] 14. ENABLER: 接入 handler registry 与 Studio executor
