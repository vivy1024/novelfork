...
1. 收尾 `platform-compliance-v1` 验证。
2. 复核 `writing-tools-v1` Tasks 1-12 已完成资产的测试与可复用状态。
3. 停止旧前端 UI 扩展。
4. 执行 `studio-frontend-rewrite` Tasks 1-4：冻结边界、新入口、布局基元、复用清单。
5. 执行 Provider / 设置页任务。
6. 执行套路页任务。
7. 执行创作工作台任务。
8. 将 `writing-modes-v1` 与 `writing-tools-v1` 的可复用 UI 挂入新创作工作台。
