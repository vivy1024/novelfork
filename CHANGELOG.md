...
NovelFork 目前仍处于**初始开发与工程验证阶段**，尚未达到正式稳定版标准。此前的全部历史版本，包括 `v3.0.0` 至 `v3.6.1`，统一追溯视为 **Beta（测试版）**：这些编号用于记录较大规模的功能演进，并不代表产品已经经历三个稳定主版本，也不构成生产级兼容性承诺。

长期开发和真实书籍验证持续暴露出基础链路缺陷；截至 Beta `v3.6.1`，经纬检索、写作管线、Runtime Overlay、数据迁移和跨平台发版仍有需要修复的问题。因此继续沿用 `v3.x` 会错误传达产品成熟度，本项目自 2026 年 8 月 11 日起重新以 `v0.0.1` 起算，并将后续 `0.x` 版本明确视为开发期版本。

历史 Beta 版本记录保留在本文件中，不删除、不改写，也不再视为正式版本线。只有在核心写作链路稳定、数据迁移与回滚经过验证、七平台发布持续可靠、主要兼容性边界明确，并完成真实用户场景验收后，NovelFork 才会进入 `v1.0.0` 正式稳定版。

## v0.0.2 (2026-08-14) — 投稿风险自检重构、叙事记忆全链路、作品级写作技能与经纬收敛

### 投稿风险自检（原平台合规）

- 平台合规重定位为**投稿风险自检**：只汇总本地敏感词线索、AI 味线索、正文完整性与连续性证据，并展示规则包标识、版本、来源与可信度。结果供作者人工复核，不代表平台审核结论。
- 移除 AI 生成比例估算：删除 `ai-ratio-estimator`、`PLATFORM_AI_THRESHOLDS` 与平台 `aiRatioTolerance`。本地 AI 味分数只是启发式文本特征提示，不表示实际 AI 生成比例，也不映射任何平台阈值。
- 移除保存硬阻断：状态由 `blocked` 改为 `needs-review`，`pipeline.write` 不再因平台口径或敏感词命中拒绝保存正文；`blockOnSensitiveBlock` 与 `publish-blocked` 一并下线。
- 每条证据带规则包标识、规则来源、章节、段落、偏移、正文摘录与人工复核建议，可回到原文定位。
- 外部检测服务评分改为「外部 AI 信号」线索，不再表述为平台拒稿风险。
- 平台 profile 收敛为软写作建议（章字数、钩子密度、自检说明），不再声称平台规则或硬门。

### 每日进度下线与写作日志收敛

- 删除每日进度卡片、`/api/progress` 与 `/api/progress/config` 接口，以及 `dailyTarget` / `weeklyTarget` 等重复目标配置；驾驶舱与全书健康度改用作品级指标（章节数、总字数、每章目标）。
- 保留 `writing_log` 底表与章节接受时的日志写入：`tools/progress` 更名为 `tools/writing-log`，语义收敛为「章节接受记录」，不再计算日目标、连续打卡与每周字数。

### 叙事记忆全链路与完整图谱工作区

- **叙事记忆全链路**：接入 LLM 事实与事件抽取、纰漏检测（`consistency-detect`）、陈旧 fact 归档提示（`staleness`）与注入预算分级治理。
- **作者自由编辑与台账落地**：前端支持 fact 编辑与状态更新，章后结算工具化并保证同章结算幂等。
- **完整记忆图谱工作区（NarrativeMemoryGraphWorkspace）**：修复原本处于未激活状态的图谱工作区，支持在主编辑器区域以独立 Tab 打开完整图谱，提供关系（Relationship）、时间线（Timeline）与浪潮（Wave）三重视图交互。

### 自研作品级写作技能（Writing Skills）与经纬收敛

- **自研 `nf-*` 技能体系**：将写作技能改为标准 `nf-*` 自研体系，支持作品级 `.novelfork/writing-skills/` 持久化存储、推荐、编辑与合规校验；技能约束同源注入 `scene.spec`。
- **经纬侧栏工具条**：经纬 UI 全面收敛至侧栏工具条，支持关联条目级联注入、同组互斥去重与剧情线状态卡。
- **伏笔与体检唯一入口**：伏笔收敛为唯一权威入口（ForeshadowingBoard），新增作品级叙事体检与健康度面板。

### 前端架构与包边界

- 前端数据层全面对齐 NarraFork 架构，统一接入 `@tanstack/react-query` 并封装标准 `api-client`。
- 将误放在 `packages/core` 的 18 个合规、过滤器与写作工具测试迁回 `packages/novel-plugin`，校准核心库边界。
- 修复 `packages/core` 存储迁移跨平台换行兼容性与测试。

### 验证与发布

- 全工作区 7 个包 TypeScript 类型检查通过。
- 七平台交叉编译一次性全部构建成功并附带 SHA256 校验和。
- 当次编译的 Windows x64 EXE 经无头隔离实例实测，启动与 `/api/health` 正常返回。

## v0.0.1 (2026-08-11) — 经纬中文检索、单环写作管线与 Runtime 边界收敛

### 经纬检索与数据一致性

- 新增经纬中文 Bigram FTS5 索引与 `0028_jingwei_search_index.sql` 迁移，支持短中文名称、别名、标签与正文检索，并为标题和别名提供更高召回权重。
- 经纬搜索会核对条目状态与更新时间，自动修复缺失、陈旧及混合时间戳导致的索引漂移；FTS 不可用时安全降级到兼容扫描。
- 修复 `tokenBudget` 排序与 SQLite 参数绑定，缺失预算统一绑定为 `NULL`，避免 Node SQLite 因 `undefined` 中断查询。

### 写作管线与 Writing Skills

- `pipeline.write` 收敛为当前 Runtime Agent 驱动的单环落盘流程，由调用方提交已生成正文，不再在工具内部平行启动重复的 Writer、Normalizer、Auditor 与 Reviser 模型链。
- Writing Skills 生效证据改由 Runtime 注入的 `loadedSkills` 提供，移除要求模型抄录长篇原文的确认硬门，同时保留缺失技能的可观测诊断。
- 强化写前检查、场景规格、正式章节落盘与错误解释，失败时返回结构化原因，避免未保存正文被误报为成功。

### Runtime 契约与旧入口清理

- 删除 Novel Plugin 旧 `/api/ai/*` 路由及 Studio 重复 AI Client，模型、会话与工具执行统一通过 NarraFork Runtime 和受控产品契约。
- 收紧书籍可信绑定、领域工具注册与工具结果 Renderer 契约，下线已废弃的 `pipeline.revise`、`style.import` 等旧工具声明及悬空渲染入口。
- 修复 Runtime Overlay 的 POSIX/Windows 路径语义测试与实例锁派生启动标记类型门禁，保持 Overlay 可重放并通过产品 parity。

### 工程与迁移说明

- 新增按 Git 变更范围选择测试和类型检查的 `run-changed-verification` 脚本，根配置、Runtime 或公共脚本变化仍自动回退到完整验证。
- 本版本首次搜索已有书籍时会自动建立经纬 FTS 索引；无需手工迁移或维护索引。
- `pipeline.write` 调用方需要提供正文 `content`；旧 `/api/ai/*` 直接调用方需要迁移到 Runtime 统一 API。
...
- 收敛卷纲、伏笔、章节摘要、角色/关系/世界设定与叙事记忆的读取/写入路径，减少文件快照、旧分类和派生状态造成的双写与漂移。
- 新增经纬权威源迁移，清理旧的卷纲摘要派生表，并补齐分类兼容、条目仓储、生命周期和读取模型测试。
- 强化章节写作资源的文件布局、迁移、附件管理与章节级资源视图，保留旧数据的兼容读取和可观测迁移结果。

### 写作链路与领域工具

- 重构 Writer、Continuity、Reviser、章节 briefing 与上下文构建，统一受控上下文、章节预算、连续性检查和审校输入。
- 完善章节读取/写入、章节审计、范围丢弃、卷纲、PGI、Pipeline Write、叙事线和经纬台账工具的契约校验与错误解释。
- 加强书籍绑定、可信根目录、领域工具权限和 Runtime 产品适配，避免模型或前端自行伪造 bookId、路径与项目身份。

### 写作工作台与结果渲染

- 重构写作工作台画布、资源树、章节编辑器、经纬条目编辑器、卷面板和 IDE 对话管理，统一章节、设定与资源的操作入口。
- 扩展 Studio 工具结果 Renderer：章节审计、记忆读取/事件/图谱、场景规格、二次模型调用等结果拥有明确的产品渲染契约。
- 补齐 Runtime 工具结果类型、Renderer 注册与嵌入式写作工作台路由测试，减少 generic fallback 和结果信息丢失。

### Core、Bridge 与 Runtime 发布门禁

- 强化 Core 状态启动、存储迁移、文件系统适配、Hook 健康、长跨度疲劳和受控上下文能力；新增对应状态、迁移、文件与可靠性测试。
- 扩展 Runtime Bridge 的窄类型契约，保持产品层不依赖 Runtime 内部实现。
- 加强 Runtime overlay manifest、补丁哈希、空操作登记、导入漂移和产品边界检查；Runtime 物化必须通过 report-only 与 parity 门禁。
- 工作区测试统一使用隔离 Runtime 数据目录、外部迁移资产和 Windows 句柄清理策略。

### 兼容性与迁移注意事项

- 本版本包含经纬权威源和写作资源迁移；升级前应保留书籍目录与 Runtime 数据库备份。
- 旧的递归摘要实现不再作为独立权威源，相关读取统一经过当前经纬/章节摘要模型。
- 版本发布前必须通过根工作区全量测试、Runtime parity、七平台编译和当次 Windows x64 EXE 核验。

## Unreleased

## v3.5.1 (2026-08-06) — Runtime 上游基线升级 v0.5.21、子代理空转重启/400 根因修复

### 🤖 Runtime 基线升级 v0.5.21
...
- **Streaming XML 闪现** — 模型输出 XML tool_use 时不再将原始 XML 文本推送给前端，消除视觉噪音

### ✨ 改进

- **状态栏超时配置** — 底部状态栏新增超时时间（秒）配置入口，点击可修改 API 首 token 等待超时
