...
- v0.1.0 Release Readiness Task 2-3：按 TDD 先在 `ConversationSurface.test.tsx` 新增 3 条 RED 组件测试，要求 `/next/narrators/:sessionId` 会话页具备 session header、runtime summary cards、session config controls、作者向空态、composer dock、recovery/confirmation lane，并确认初始运行 1 file / 26 tests 中 3 failed / 23 passed，失败原因是当前 DOM 缺少对应成品化分区。随后重排 `ConversationSurface`、`ConversationStatusBar`、`Composer` 和 `ConversationRuntimeControls`，形成 Session Header、Runtime Summary Cards、Recovery/Confirmation Lane、Message Stream、Composer Dock 五区布局，使用 `paper-sheet` / `glass-panel` / Tailwind token，不引入 Mantine、不恢复旧 ChatWindow，并保留 session config 更新、工具卡 raw 脱敏、审批、compact、abort 与 slash command 合同。验证 `ConversationSurface.test.tsx` 通过（1 file / 26 tests），`ConversationSurface.test.tsx` + `StudioNextApp.test.tsx` + `src/api/routes/session.test.ts` 通过（3 files / 72 tests），client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 4：扩展 `e2e/settings-session-conversation.spec.ts`，用隔离 `.novelfork/e2e-workspace-flow-*` root、真实 provider/settings/session/chat state API 覆盖会话页空态五区布局、模型不可用恢复说明与设置入口、composer dock slash 建议、工具卡 raw 脱敏与全屏详情、pending confirmation lane + 拒绝后 `/api/sessions/:id/tools` 回读清空、模型/权限/推理配置回读和 idle/running 中断控制；先以 Playwright RED 暴露空态缺“模型池为空，请先到设置页启用模型”，再让空态同步展示 `sendDisabledReason`。验证 `pnpm exec playwright test e2e/settings-session-conversation.spec.ts` 通过（3 tests passed），不调用真实模型。
- v0.1.0 Release Readiness Task 5：为叙述者中心与 Shell 左栏补 RED 测试，新增 `SessionCenter.test.tsx`、`NewSessionDialog.test.tsx`、`AgentShell.test.tsx` 失败优先用例，锁定会话中心缺工作目录/创建时间/最后消息时间/排序控件、新建独立叙述者缺工作目录/绑定对象/模型选择、Shell 左栏未限制最近会话且缺“查看全部叙述者”入口。验证 `pnpm --dir packages/studio exec vitest run src/components/sessions/SessionCenter.test.tsx src/components/sessions/NewSessionDialog.test.tsx src/app-next/shell/AgentShell.test.tsx` 得到 3 failed / 15 passed，失败均来自 Task 6 待实现的 UI/行为缺口。
- v0.1.0 Release Readiness Task 6：完成发布级叙述者中心与 Shell 左栏收敛，`SessionCenter` 新增工作目录、创建/最后消息时间、排序控件和 `NewSessionDialog` 新建入口，`NewSessionDialog` 支持工作目录、绑定对象、模型选择、权限模式与计划模式，`ShellSidebar` 只展示前 5 条活跃会话并提供“查看全部叙述者”进入 `/next/sessions`，`shell-route` / `StudioNextApp` / session API sort 支持该入口。验证 Task 5 RED 转 GREEN（3 files / 18 tests passed），邻近回归 9 files / 87 tests passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 7-8：为 `/next` 产品壳首页补 RED 测试并实现发布级作者首页。`routing.test.tsx` 新增有数据与空态首页用例，先失败于主区仍为 `Agent Shell` 占位页；随后 `StudioNextApp` 新增 `HomeRouteLive`，复用 `useShellData` 的 books/sessions/provider summary/status 展示最近作品、最近会话、模型健康、快速动作（新建/继续会话、设置、套路库）和空态，不硬编码假统计。验证 routing 聚焦 1 file / 4 tests passed，routing/StudioNextApp/entry/useShellData/AgentShell 邻近回归 5 files / 39 tests passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 9：为作品工作台发布级体验补 RED 测试。新增 `WritingWorkbenchRoute.test.tsx`，要求 `/next/books/:bookId` 工作台首屏显示作品标题、当前状态和资源树/当前资源画布/写作动作区域；扩展 `WorkbenchCanvas.test.tsx`，要求资源 header 卡片展示资源类型、真实路径、读写能力、保存状态和只读原因；扩展 `WorkbenchWritingActions.test.tsx`，要求动作卡展示 `session → candidate`、`prompt-preview`、`audit` 结果边界。验证聚焦 Vitest 得到 3 failed / 12 passed，失败均为 Task 10 待实现的发布级解释层缺口。
- v0.1.0 Release Readiness Task 10：完成作品工作台体验成品化。`WritingWorkbenchRoute` 新增作品工作台 header、资源加载/选择状态和资源树/当前资源画布/写作动作区域语义；`WorkbenchCanvas` 新增 `workbench-resource-header`，展示资源类型、真实路径、读写能力、保存状态和只读原因，同时保留 dirty/save/hydrate guard；`WorkbenchWritingActions` 从 writing action descriptor 的 `outputBoundary` 派生作者可理解结果边界，不改变 session 创建/复用逻辑。验证 Task 9 RED 转 GREEN（3 files / 15 tests passed）、workbench 邻近回归 9 files / 62 tests passed、`e2e/workbench-resource-edit.spec.ts` 浏览器 E2E 1 passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 11：治理设置 / Provider 可读性与 E2E 夹具污染。`ProviderSettingsPage` 新增搜索供应商或模型、只看异常项、隐藏测试夹具控件，并在 API key provider 卡片标记 E2E Provider/Model 为测试夹具开发数据；`SettingsTruthModel` 新增 `deriveProviderFixtureFacts`，将测试 provider 派生为 providers 组清理事实而非普通 current 数据；`e2e/settings-session-conversation.spec.ts` 在真实浏览器中验证 E2E Provider 可见为测试夹具且可隐藏。验证 Provider/Truth 聚焦 2 files / 22 tests passed，Settings 目录 6 files / 37 tests passed，Playwright 3 passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 12：验活 Routines / 工作流配置台发布口径。现有 `/next/routines` 已通过 routines API 与 MCP registry 展示命令、可选工具、工具权限、全局/项目技能、自定义子代理、全局/系统提示词、MCP 工具和生命周期钩子等十个分区；新增 `e2e/routines-workflow.spec.ts` 真实浏览器打开 `/next/routines`，覆盖分区入口、MCP Server 管理、策略来源和钩子入口，并确认无假 current 文案。验证 Routines 聚焦 1 file / 4 tests passed，Playwright 1 passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 13：建立 clean/isolated root 主路径浏览器 E2E。新增 `NOVELFORK_RUNTIME_DIR` 覆盖 runtime storage，使 Playwright 的 provider/user runtime state 与 session store 一起隔离到 `.novelfork/e2e-workspace-flow-*/.runtime/`，先 RED 暴露 clean project root 仍读取用户级 E2E Provider 污染，再修复 runtime 路径；新增 `e2e/release-readiness-main-path.spec.ts` 覆盖 clean root 无 E2E/Planner 噪声、`/api/mode`、`/`、`/next`、作品/工作台、写作动作创建 session、会话中心/会话页、设置/Provider/关于与 Routines；同步收敛旧 `workspace-creation-flow` 到当前 Workbench 路径。验证 runtime-storage-paths 1 file / 3 tests passed，完整 Playwright 7 passed，client/server TypeScript 通过。
- v0.1.0 Release Readiness Task 14：完成 clean root 手工软件验活并补首页新建作品入口。使用 root `D:\DESKTOP\novelfork\.novelfork\manual-release-smoke-v0-1-0-20260507-1900`、API `http://127.0.0.1:4597`、Vite dev `http://127.0.0.1:4598` 实际打开 `/api/mode`、`/`、`/next`、作品创建、工作台资源 header、自动写作会话、会话中心、设置、Provider、Routines、关于；手工发现 `/next` 只能新建会话、不能创建作品，按 TDD 补 routing RED 并在 `HomeRouteLive` 增加“新建作品”表单，走真实 `/api/books/create`，成功后刷新 books 并导航到工作台。记录同步到测试状态与 NarraFork UI 调研；embedded `bun run main.ts` 暂仍读取旧嵌入资产，后续编译/release smoke 必须重建后再验。
- v0.1.0 Release Readiness Task 15：收口 `ui-live-parity-hardening-v1` 最终任务状态。`ui-live-parity-hardening-v1` Task 14 已标记完成，`.kiro/specs/README.md` 同步为 14/14 完成；release-readiness 进度推进到 15/20，并明确剩余门槛进入全量自动化验证、版本资料、编译产物、GitHub Release 与归档发布步骤。
- v0.1.0 Release Readiness Task 16：完成全量自动化验证矩阵。先治理审计暴露的 app-next `/api/*` 路径散落，将 user settings、proxy、provider models/status/summary、book create 与 worktree status 路径集中到 Backend Contract `api-paths` 常量/helper 并更新测试夹具；同时保持 SettingsTruthModel、Agent Shell + Writing Workbench、session runtime 边界，修复占位文案和 Sidebar 嵌套 button 回归。验证覆盖 app-next 聚焦 47 files / 271 tests、Backend Contract 7 files / 38 tests、Studio typecheck、CLI 13 files / 97 tests、docs verify、Playwright 7 tests 和 diff check；CLI 并行矩阵首次 2 个 timeout 经隔离重跑确认为资源争用而非功能回归。
- v0.1.0 Release Readiness Task 18：完成 Windows 单文件编译与真实 compiled smoke。`pnpm --dir packages/studio compile` 生成 `dist/novelfork-v0.1.0-windows-x64.exe`（约 115M）与 SHA256 `d0eb2bf63385ba6c5695485de129c20bacda28dd54c22ad7647f9478292bee7d`；首次 smoke 暴露 startup compile-smoke 仍按 clean root `dist/` 查找 exe，已新增 `compile-smoke.ts` 与 helper 测试，改为 compiled runtime 使用当前 exe path 判断单文件产物。最终用 clean root + `NOVELFORK_RUNTIME_DIR` 启动真实 exe，验证 embedded assets、release metadata、startup delivery、Provider clean 状态、作者首页、UI 创建作品进入工作台与 Routines 页面；Task18 不使用 mock 单测作为发布验活证据。

### 文档
- 重构 steering 与 Superpowers 项目适配层：将 `.kiro/steering/` 旧六件套整合为 `README.md` 路由索引与 `project-profile.md` 项目事实/完成标准，删除重复的 project-rules、project-standards、git-workflow、agent-pipeline、writing-workflow 与 mcp-tools-reference 文件；新增 `novelfork-project-profile` skill，并同步 `CLAUDE.md` / `AGENTS.md` 改为通过 `using-project-steering`、`kiro-spec-adapter` 与 `feature-closure-gate` 路由项目规则，避免通用 Superpowers 被项目特定规则污染。
- 新增 `v0-1-0-release-readiness` Kiro spec（requirements/design/tasks）：用户明确要求“完整功能才发布 v0.1.0”，因此将产品壳入口、作品工作台、叙述者中心、会话页、设置/Provider/Routines、干净 root 实际软件验活、E2E 夹具治理、旧 spec 收口归档、版本资料、编译 smoke、Git tag 与 GitHub Release 产物统一纳入 v0.1.0 发布阻塞门槛；明确 Task 13 E2E passed 只能证明路径可运行，不能证明 UI 体验达到 NarraFork 成熟度；完整功能、真实 UI 体验、干净 root 验活和 Release 产物全部达标前不得发布 v0.1.0；tasks.md 原拆 20 项，2026-05-07 用户反馈后扩展为 23 项，在 release 前新增 Claude/Codex 源码对照、mock/hardcoded 清理与重新验收门槛；按发布准备基线、会话页、叙述者中心、首页、工作台、设置/Provider、Routines、clean root 验活、归档、版本、compile、源码/质量重审、重新验收、Release 顺序执行。完成 Task 1，新增 `release-readiness-baseline.md` 记录 approved spec、ui-live-parity Task 14、4567/7778 手工 UI 对比、能力矩阵、测试状态、执行前干净工作树、当前 0.0.6 版本号、dist 无 v0.1.0 产物，以及 Requirement 1-11 → Task/子系统/验证映射和当时发布阻塞项；后续已新增 Requirement 12。
- UI Live Parity Hardening Task 14（进行中）：追加 2026-05-07 真实 UI 交互体验记录，同机浏览器访问 NovelFork `http://127.0.0.1:4567` 与 NarraFork `http://127.0.0.1:7778`，记录设置页、工作台/项目页、叙述者列表与会话页差异；确认 NarraFork 7778 当前可见 `v0.4.1`、提示 `更新: 0.4.1 → 0.4.2`，其 `/settings`、`/projects`、`/narrators` 与会话页在信息架构、Mantine 视觉壳、筛选/排序/归档、运行透明度上更成熟；NovelFork 在 SettingsTruthModel、provider callable、资源树和会话合同上已更真实，但 `/next/narrators/:sessionId` 手工观感仍像基础 HTML，标题/状态/控件挤在顶部、composer 贴底且空白过大，左栏会话长列表缺筛选/归档管理，本机还残留多批 E2E Provider 测试数据。因此文档新增“体验未达标”口径，明确 Task 13 E2E passed 只证明路径可运行，不等于 v0.1.0 UI 体验达 NarraFork 成熟度。
