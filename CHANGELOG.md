...
- Frontend Refoundation：将 `/next` 入口切到 Agent Shell 路由壳，并让 Shell 首屏数据通过 Backend Contract domain clients 读取 books、sessions 与 provider 状态；补充合同 client 对 session/resource/provider/writing action 的显式 helper 与类型约束，继续保留 `prompt-preview`、`process-memory`、`unsupported` 等能力状态；`backend-contract-v1` 验收后正式计入前端进度并完成 12/12 验收收口；`/next/narrators/:sessionId` 挂载单栏 Conversation、`/next/books/:bookId` 挂载 Writing Workbench 壳，不再停留在纯占位路由；Conversation runtime 已通过合同快照 hydrate 消息/session/cursor，以 `resumeFromSeq` 建立 WebSocket，并支持解析 message/stream/error 与发送 ack/message/abort envelope；Conversation surface 已补齐 flex-column 单栏布局、消息流、工具卡、确认门、状态栏、Composer 与内联 recovery notice；模型/权限状态栏动作已支持 provider/model、permission、reasoning、usage、session config update 失败提示、模型池为空禁发与 unsupported-tools 降级；Tool Result Renderer Registry 已接入 cockpit、questionnaire、pgi、guided、candidate、narrative、generic renderer 与 artifact 打开动作；Writing Workbench 资源树已改为消费 resource contract adapter 节点，覆盖章节、候选稿、草稿、经纬、story/truth、叙事线、unsupported 与 read/edit/delete/apply capability 映射；Workbench canvas/viewer 已支持资源打开、保存章节/草稿、只读禁用、tool-result viewer 与 dirty canvasContext 输出；Workbench 写作动作入口已从 writing action descriptors 生成，支持创建或复用绑定 bookId 的 session、跳转 Conversation 与 unsupported 禁用说明；主入口已用 StudioNextApp 接管并增加 route guard，避免 `/next` 默认入口回退旧三栏 WorkspacePage；旧 StudioApp、WorkspacePage、SplitView、EditorArea、ConversationPanel、useStudioData 与 ChatWindow floating/docked 入口已通过 tsconfig exclude 从 Studio typecheck 构建路径正式退役，并补充 retirement guard；阶段收口已运行 app-next Vitest、Studio/Core TypeScript、docs verify 与 `/next` 浏览器冒烟。Backend Core Refactor 已建立 `contract-guard.md`，冻结 route/tool/shared type 映射、不可破坏合同分组和 task 2 补测清单；新增 `contract-regression.test.ts` 覆盖 books、sessions、providers、resources、writing actions 的成功、404/400、unsupported/gate 与脱敏边界。
