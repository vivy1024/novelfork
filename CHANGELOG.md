...
- Frontend Refoundation：将 `/next` 入口切到 Agent Shell 路由壳，并让 Shell 首屏数据通过 Backend Contract domain clients 读取 books、sessions 与 provider 状态；补充合同 client 对 session/resource/provider/writing action 的显式 helper 与类型约束，继续保留 `prompt-preview`、`process-memory`、`unsupported` 等能力状态；`backend-contract-v1` 验收后正式计入前端进度（3/12），并让 `/next/narrators/:sessionId` 挂载单栏 Conversation、`/next/books/:bookId` 挂载 Writing Workbench 壳，不再停留在纯占位路由；Conversation runtime 已通过合同快照 hydrate 消息/session/cursor，以 `resumeFromSeq` 建立 WebSocket，并支持解析 message/stream/error 与发送 ack/message/abort envelope。
