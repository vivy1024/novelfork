...
## v0.0.2 (2026-08-12) — 投稿风险自检重构、每日进度下线与包边界校准
...
- 将误放在 `packages/core` 的 18 个合规、过滤器与写作工具测试迁回 `packages/novel-plugin`，并移除 core 的 `./compliance` 导出与相应 vitest 包含规则。
- 平台 profile 收敛为软写作建议（章字数、钩子密度、自检说明），不再声称平台规则或硬门。

### 文档

- 学习文档与生成的学习内容同步为投稿风险自检口径，明确规则来源、可信度与人工复核边界。
