...
### 任务
- [x] 新建正式 `NewProjectDialog` / `ProjectCreate` 流程（第一轮已落独立对象流与入口）
...
- `NewProjectDialog` / `ProjectCreate` 独立对象流已落地，入口不再只沿用 `BookCreate`。
- 当前仍未完成真正的 clone / git / worktree 执行动作，项目初始化与仓库操作仍以预留字段和后续接线为主。
...
- [x] 增加计划模式启动（第一轮已支持 planner 默认计划模式与手动切换）
- [x] 增加归档 / 恢复 / 分类筛选 / 排序（第一轮已落会话中心归档/恢复/筛选/排序）
- [x] 建立服务端会话生命周期接口（第一轮 CRUD 已落地）
- [x] 建立 per-session chat / ws 协议（第一轮已落最小可用 session chat transport）
- [~] 把 `ChatWindow` 改成真正消费服务端 session 状态的控制台（已接入服务端快照/状态优先，但仍非完整远端事实源）
...
- `NewSessionDialog.tsx` / `SessionCenter.tsx` 已支持 planner 默认计划模式与会话级 `chat / plan` 启动切换。
- `session-chat-service.ts` / `start-http-server.ts` 已提供 per-session chat + ws 的最小可用 transport，并支持 snapshot / state / message 三类事件。
- `ChatWindow.tsx` 已优先消费 `/api/sessions/:id/chat/state` 与 session chat transport，下沉到服务端 session 快照与状态更新。
- 当前仍未完成更彻底的“ChatWindow 全量服务端状态驱动”与持久化会话消息历史；现阶段仍保留窗口态与内存 transport 的混合实现。
...
- [x] 增加子代理卡片（第二轮已在 ToolCallBlock 落地）
...
- `ToolCallBlock.tsx` 已按工具类型差异化展示，并增加耗时、错误摘要、复制操作与子代理卡片。
- 当前仍未完成更完整的全屏 / 重跑 / 查看源码类操作。
...
- [x] 把 daemon / logs / worktree 从占位合并进管理中心体系（第二轮已收口到 Admin）
...
- `DaemonTab` / `LogsTab` / `WorktreesTab` 已并入 Admin，系统运维入口第一轮已经完成统一收口。
- 当前仍未完成 terminal / container 等更深一层的系统运维入口。
...
- [x] 增加默认模型 / 摘要模型 / 子代理模型池（第二轮已接入主配置流）
- [x] 增加默认权限模式 / 推理强度 / 上下文阈值
- [x] 增加恢复 / 重试 / 退避配置（第二轮已接入类型、默认值、持久化与面板）
- [x] 增加工具白名单 / 黑名单 / MCP 策略入口（第二轮已接入主配置流）
- [x] 增加运行调试项（token、速率、dump、trace 等分阶段接入）（第二轮已完成第一批配置入口）
...
- `RuntimeControlPanel.tsx` 已把默认会话模型、摘要模型、子代理模型池、默认权限模式、默认推理强度、上下文压缩阈值、上下文截断目标接入 `/api/settings/user`。
- 恢复 / 重试 / 退避、工具白名单 / 黑名单 / MCP 策略、运行调试项（token / rate / dump / trace）也已进入统一类型、默认值、后端持久化与前端面板保存链路。
- 这组字段现在已真正影响 session 默认值与 context manager 阈值，不再是只保存不生效的死配置。
- 当前仍未完成把这些新配置真正接入执行器、工具调度器与日志/trace 管线；现阶段完成的是配置层与主控制面板层。
...
### 任务
- [~] 继续验证和收敛 Bun 单入口方案（当前已明确 `main.ts` 为源码主入口，CLI fallback 已收口为 main.ts → 已安装包 → legacy source bridge）
- [~] 前端资产内嵌与本地 HTTP 服务模型明确化（embedded/filesystem 双路径已梳理，正式分发口径仍未闭环）
- [~] 启动期迁移 / 索引恢复最小 orchestrator 已接入 `startStudioServer()`（best-effort，仍非持久化巡检）
- [x] 启动期迁移、索引恢复、状态修复任务整理成正式清单（见 `docs/06-部署运维/03-启动期迁移与修复清单.md`）
- [ ] `bun compile` 单文件分发链路落地
- [~] CLI spawn studio 仅保留过渡职责（当前已优先转交 Bun 主入口，package fallback 收口，legacy source bridge 仅最后兜底）
...
- [~] ChatWindow 成为真正的会话控制台，而不是前端假对象（已接入服务端状态优先，但仍未完全去除本地窗口态依赖）
- [ ] 上下文 / 工具 / 子代理 / 模型 / 权限 / 推理全部在会话页透明可见
- [x] Routines 与 WorkflowWorkbench 已统一为一个编排工作台
- [x] Requests / Providers / Resources / Storage 已具备真实运维价值
- [~] Settings 成为运行策略控制面板（配置层已基本收口，执行器/调试链接线仍待继续）
- [~] MCP/tool registry 进入系统级管理（已补统一注册表视图、连接状态、工具数与编辑能力，权限体系尚未完全纳管）
- [~] 运行时与分发路径明确向本地单体应用收敛（main.ts / fallback / startup orchestrator / bun compile smoke 已推进，正式分发仍未闭环）
