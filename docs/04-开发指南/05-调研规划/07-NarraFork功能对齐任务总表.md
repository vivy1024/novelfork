...
**创建日期**: 2026-04-20  
**更新日期**: 2026-04-21  
**状态**: 🔄 执行中（A/B/C 主链已完成并提交；兼容层已清仓，进入 D/G/H 主线与 B/C 深化并行阶段）
...
- [ ] `ProjectCreate / BookCreate` 的产品化文案、创建中状态与错误反馈
- [ ] bootstrap 之后的首个默认工作流（例如自动打开项目 / 会话 / 初始化提示）
...
### 半完成 / 仍有缺口
- [~] `ChatWindow` 已转向 server-first，但仍未完全达到“纯服务端事实源”终态
