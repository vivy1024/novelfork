...
### 当前进度（2026-04-20，本轮已推进）
- `SessionCenter.tsx` 与 `ChatWindow.tsx` 的“新建会话 / 新开会话”已先创建正式 session，再打开窗口。
- `windowStore.ts` 已从纯窗口对象扩展为“窗口 + session 关联”的过渡态。
- 当前仍属于第一轮打底：主列表统计、运行时消息同步、配置回写、归档恢复与 per-session ws 协议还没有完全切到服务端事实源。
...
### 任务
- [ ] 精确化上下文 token 统计
- [ ] 增加会话 breadcrumb / 项目路径
- [ ] 做上下文来源分层（session/tool/project/memory 等）
- [ ] 细化 ToolCallBlock 对 Bash / Read / Write / Browser / MCP 的专门展示
- [ ] 增加子代理卡片
- [ ] 增加逐步耗时、错误态、更多操作（复制 / 查看源码 / 全屏 / 重跑）
...
- [ ] 增加 Prompt / Tool Permissions / Sub-agents / Hooks 正式区块
...
- `WorkflowWorkbench.tsx` 已加入 routines 主事实源说明块，但真正的 prompts / permissions / sub-agents / hooks 统一工作台入口还需第二轮继续并入。
...
### 任务
- [ ] 请求历史接入真实 provider/model/token/TTFT/cost/cache/narrator 维度
...
- 当前仍未完成 requests / resources / storage / daemon 等运维维度的真实化，这部分仍是下一轮重点。
...
- 当前仍未完成模型池、默认权限、恢复退避、工具白名单等运行策略项，这部分还需要继续扩展设置页字段与后端配置结构。
...
### 任务
- [ ] 继续验证和收敛 Bun 单入口方案
- [ ] 前端资产内嵌与本地 HTTP 服务模型明确化
- [ ] 启动期迁移、索引恢复、状态修复任务整理成正式清单
- [ ] `bun compile` 单文件分发链路落地
- [ ] CLI spawn studio 仅保留过渡职责

### 重点文件
- 以 `docs/04-开发指南/05-调研规划/01-平台迁移方案.md`
- `02-平台回正规划.md`
- `03-平台迁移待办清单.md`
- 以及 `packages/studio` 启动入口为准
