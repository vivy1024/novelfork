...
### 3.2 Session / Narrator 第一轮内核化（已完成主链，仍未彻底）
...
仍未完成：
- [ ] `ChatWindow` 仍然依赖 `windowStore`，还不是纯服务端事实源
...
当前状态：
- [x] 已完成第一轮主链统一
- [ ] 仍需继续削弱 `windowStore` 等 UI 层对 session 主事实的占有
...
当前已完成：
- [x] `NewProjectDialog` / `ProjectCreate` 第一轮入口
- [x] 仓库来源 / 工作流模式 / 模板偏向 / worktree 预留字段

仍未完成：
- [ ] 真正的 clone / git / worktree 执行动作
- [ ] 项目创建后自动进入“可工作状态”的完整初始化链

## Phase C：Narrator / Session 内核化（当前主线之一）

总体判断：
- [~] 已完成主链，但离真正 NarraFork 级 narrator/session 平台还有距离

剩余重点：
- [ ] 继续把 `ChatWindow` 推向纯服务端状态优先，进一步弱化本地窗口态
- [ ] 持久化完整聊天历史，而不只是最近消息
- [ ] 继续统一会话恢复、重连、状态恢复策略
...
1. **继续收口 ChatWindow → 服务端事实源**
   - 这是 Session 真正完成内核化的关键
2. **把 Settings / MCP 策略接进执行链**
   - 让配置从“能保存”变成“真生效”
3. **补运行时正式分发闭环**
   - 让平台从“开发可跑”进入“产品可交付”
4. **补 terminal / container / 更强 Admin 实时性**
   - 完成平台运维最后一层
