---
title: 设置与套路
summary: 配置 AI 供应商连接和模型路由，用套路系统定义 Agent 行为边界
tags: [设置, 供应商, 套路, 模型聚合, MCP]
routes:
  - /next/settings
  - /next/settings/providers
---

# 设置与套路

> 配置 AI 供应商连接和模型路由，用套路系统定义 Agent 行为边界。

## 核心概念

- **供应商**：AI 模型的 API 连接配置（Key + Endpoint + 可用模型列表）
- **模型聚合**：多供应商的同名模型合并为一个逻辑模型，按策略路由请求
- **套路（Routines）**：Agent 行为规则集——命令、工具、权限、技能、子代理、提示词、MCP 七大维度
- **全局 vs 项目**：全局套路对所有作品生效，项目套路覆盖全局

## 推荐使用流程

1. 进入设置 → 供应商，添加至少一个 API 供应商（填 Key + Base URL + 拉取模型列表）
2. 设置默认会话模型、摘要模型、子代理模型池
3. 如有多供应商提供同一模型，启用模型聚合并选择路由策略（priority / round-robin / random）
4. 进入套路页面，配置全局套路的命令、工具、权限基线
5. 在具体作品中按需覆盖项目套路（题材规则、专用技能、MCP 工具）

## 最佳实践

- 先配好供应商再做其他事，没有可用模型一切功能不可用
- 全局套路放通用规则（权限从严、基础命令），项目套路放题材定制
- 模型聚合用 priority 策略时把最稳定的供应商排第一，故障自动切换
- MCP 工具按需启用，不用的关掉减少 Agent 工具列表噪音
- 权限建议从 ask-always 开始，熟悉后再切 auto-approve

## 常见坑

- **模型列表为空** → API Key 过期或 Base URL 错误，先用"测试连接"按钮验证
- **命令不生效** → 检查套路命令 Tab 是否被禁用，项目套路会覆盖全局
- **MCP 工具超时** → MCP 服务器未启动或网络不通，查看监控面板的请求日志
- **聚合路由不切换** → 只有请求失败才触发 fallback，手动切换需改 priority 顺序

## Agent 查阅提示

- 供应商配置存储在 settings store，通过 `useSettingsStore` 读取
- 套路合并逻辑：全局 + 项目，同名项目优先；提示词拼接（全局在前）
- 会话创建时套路注入 sessionConfig：commands / tools / permissionMode / skills / subAgents / systemPrompt
- 模型聚合配置在供应商设置中，路由策略影响 LLMClient 的请求分发

## Hooks 系统

套路支持三种 shell 钩子，在 Agent 操作生命周期中自动触发：

| 钩子 | 触发时机 | 典型用途 |
|------|---------|---------|
| PreToolUse | 工具调用执行前 | 参数校验、日志记录、拦截危险操作 |
| PostToolUse | 工具调用执行后 | 结果后处理、通知推送、审计日志 |
| TurnComplete | 一轮对话结束后 | 自动提交、同步状态、触发下游流程 |

钩子脚本在套路的 Hooks tab 中配置，支持任意 shell 命令。脚本退出码非 0 时阻止后续操作（PreToolUse）或记录警告（PostToolUse/TurnComplete）。

## MCP 工具扩展

通过连接外部 MCP Server 为 Agent 注入额外工具能力：

- 在套路的 MCP tab 中添加 MCP Server 地址（本地 stdio 或远程 HTTP）
- 连接成功后，Server 暴露的工具自动出现在 Agent 工具列表中
- 支持按套路粒度启用/禁用特定 MCP 工具
- MCP 工具与内置工具共享权限模式和确认门机制

## 多用户认证

支持三种认证模式，适配不同部署场景：

| 模式 | 说明 |
|------|------|
| none | 无认证，单用户本地使用 |
| builtin | 内置用户名密码认证，适合小团队 |
| external | 对接外部 OAuth/SSO，适合企业部署 |

认证模式在设置 → 安全 tab 中切换。切换后需重启服务生效。

## v0.9.x 新增配置

### 服务器配置

在设置 → 服务器与系统中可配置：

- **端口和监听地址**：默认 127.0.0.1:4567，改为 0.0.0.0 可公网访问
- **TLS/HTTPS**：配置证书路径启用加密连接
- **浏览器打开方式**：启动时自动打开浏览器/应用窗口/不打开
- **更新检查**：检查 GitHub Release 是否有新版本

### 运行时控制

在设置 → AI 代理中可配置：

- **首 token 超时**：API 请求超时后自动中断（0=禁用）
- **自定义重试规则**：将特定错误标记为可重试（按 HTTP 状态码或内容关键词匹配）
- **沙箱模式**：none/basic/strict 三级隔离
- **Hooks**：工具执行前后自动触发 shell 命令
- **自动验证**：Write/Edit 后自动运行 typecheck 等验证命令

### 套路系统新增

- **全局提示词**：在套路页添加的全局提示词会注入到所有叙述者的 system prompt 中
- **自定义子代理**：定义专用子代理类型，配置 systemPrompt 和工具权限
- **自定义斜杠命令**：定义 `/命令名` → prompt 模板，在 Composer 中 `/` 触发补全

## 可跳转功能入口

| 功能 | 路径 |
|------|------|
| 设置主页 | `/next/settings` |
| 供应商配置 | `/next/settings/providers` |
