...
**PGI（Pre-Generation Interview）**：生成前追问。AI 在动笔前先问 2-5 个问题，明确你对这章的期望——情绪基调、关键事件、字数要求等。

**pgi.ask**：PGI 单一工具（v1.7.0 起合并了旧的 generate_questions / record_answers / format_answers_for_prompt 三步）。一次调用完成：基于经纬数据生成追问 → 返回可供 AskUserQuestion 展示的问题卡。
...
```
用户请求（写下一章）
  → cockpit.snapshot（了解进度/伏笔/候选稿状态）
  → jingwei.read(scope=brief)（读经纬核心包）
  → pgi.ask（生成追问）
  → AskUserQuestion（展示给用户，整个流程只一次）
  → 用户回答
  → scene.spec（生成结构化蓝图）
  → jingwei.read(scope=category)（按蓝图补读相关经纬）
...
5. 蓝图符合预期 → AI 执行 `pipeline.write` 生成候选稿
...
- 整章生成走 `pipeline.write`（传入 sceneSpec），产出候选稿
