...
| 路径 | 作用 | 当前边界 |
|---|---|---|
| `PUT /api/books/:id/beat-template` | 持久化用户选择的节拍模板 | Body: `{ beatTemplateId: string }`；写入书籍配置 |

### 文风仿写

| 路径 | 作用 | 当前边界 |
|---|---|---|
| `POST /api/books/:id/style-profile` | 上传范文，提取风格特征 | Body: `{ content: string, name?: string }`；Response: `{ ok: true, profile: { name, stats, promptInjection } }` |
| `GET /api/books/:id/style-profile` | 获取已提取的风格特征 | Response: `{ profile: {...} \| null }` |

### 章节自动链接

| 路径 | 作用 | 当前边界 |
|---|---|---|
| `POST /api/books/:bookId/chapters/:ch/link` | 触发自动链接扫描 | Body: `{ content?: string }`（可选，不传则从文件读取）；Response: `{ links: [{ entryId, title, matchedText }] }` |
| `GET /api/books/:bookId/chapters/:ch/links` | 获取该章关联条目 | Response: `{ links: [...] }` |

### 会话工具策略

| 路径 | 作用 | 当前边界 |
|---|---|---|
| `PUT /api/sessions/:id` | 更新 session（含 toolPolicy） | Body: `{ sessionConfig: { toolPolicy: { deny: string[] } } }`；更新会话配置并持久化 |
