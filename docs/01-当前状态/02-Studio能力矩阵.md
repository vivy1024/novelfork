...
| `/next/narrators/:sessionId` live 叙述者会话入口 | ✅ Task 9-10：header facts 来自真实 session record 与 worktree status API；模型/权限/推理更新后回读 session snapshot；工具/审批/运行控制透明化不冒充未接能力 |
| 13 个 Core Agent 类 | ✅ |
