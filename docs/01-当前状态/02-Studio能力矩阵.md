...
| 中间画布 | 多资源 Tab、章节编辑、候选稿、经纬详情、工具产物、dirty 切换拦截；`ui-live-parity-hardening-v1` Task 2-4 已补资源详情 hydrate、保存控制器、dirty guard 与 Playwright 浏览器保存刷新读回；v0.1.0 Release Readiness Task 9 已补作品工作台发布级 RED，锁定作品标题/状态、资源 header 卡片、只读原因和写作动作结果边界缺口 | ⚠️ 资源 hydrate/保存防覆盖/浏览器读回已验证；发布级解释层待 Task 10 转 GREEN |
...
| 作品工作台解释层 | v0.1.0 Release Readiness Task 9 已新增 RED：工作台首屏必须展示作品标题/当前状态/清晰区域，资源 header 必须卡片化展示类型/路径/读写能力/保存状态/只读原因，写作动作必须显示 session/candidate/draft/audit/prompt-preview/unsupported 结果边界；当前 3 failed / 12 passed，待 Task 10 实现。 |
