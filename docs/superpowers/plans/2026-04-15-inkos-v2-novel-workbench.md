...
- [ ] Tab 3：**节奏波形图**（3+1 分析，红线=高压，蓝线=缓冲）。
...
- [ ] 设定防膨胀守护进程：每周分析 `memory.db`，标红未使用的冗余设定。
...
- [ ] **字数与节奏控制**：为批量输出模式配置日更 4k-10k 目标面板与 5 分钟人工查阅卡点。

### Phase 6：会话持久化与 Pipeline 面板（2-3 天）

目标：极致的工程体验。
- [x] IndexedDB 接入，防止刷新丢草稿；恢复打开的 Tabs。
- [ ] `PipelinePanel.tsx` SSE 实时更新替代 polling（现有 polling 已可用）。

### Phase 7：MCP、Plugins 与搜索服务（2 天）

目标：开放性与寻址能力。
- [ ] MCP 客户端页面真实接通，管理外部工具。
