...
番外写作 + 文风仿写 + 写后验证器 + 审计闭环加固 + Writer 两阶段拆分 + 多提供商路由。
...
Writer 单次 LLM 调用拆成两阶段：创作（temp 0.7, 8192 tokens）只输出正文，结算（temp 0.3, 8192 tokens）只更新 truth 文件。创作用高温度保持创造力，结算用低温度减少幻觉。章节上下文从最近 3 章全文压缩为最近 1 章 + 历史摘要。

### 多提供商路由

不同 agent 走不同 LLM 提供商（不同 baseUrl、API Key、流式设置），Writer 用贵模型保质量，Auditor 用便宜模型省成本：
...
- 非流式 LLM 支持（per-provider 可配 `stream: false`）
- 错误诊断修复：不再把所有异常吞为 "inkos.json not found"
...
| `inkos config set-model <agent> <model>` | 为指定 agent 设置模型覆盖（`--base-url`/`--provider`/`--api-key-env`/`--no-stream`） |
...
- [x] 去 AI 味铁律 + 文风指纹注入
- [x] 多 LLM provider（OpenAI + Anthropic + 兼容接口 + per-agent 提供商路由）
- [x] AIGC 检测 + 反检测改写管线
