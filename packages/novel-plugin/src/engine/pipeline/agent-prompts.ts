...
- ⚠️ layer 选择规则（必须严格遵守）：
  - canon 层：**只用于永恒不变的世界规则**——物理法则、历史事实、世界观底层设定（如"灵气复苏发生在公元 2000 年"、"金丹期以下不可飞行"）。这些内容一旦确立就绝不会因剧情发展而改变。
  - dynamic 层（默认）：角色信息、角色状态、事件记录、关系变化、势力动态、伏笔、大纲、章节摘要等**会随剧情发展而变化**的内容。绝大多数写入应使用 dynamic 层。
  - reference 层：外部参考资料、灵感素材等非正式设定内容。
  - 判断标准：如果这条信息可能因为后续剧情而需要更新（角色升级、关系变化、事件发生），就必须用 dynamic。只有"宇宙级别的不变规则"才用 canon。
