...
export type WritingResourceRepository = {
  readonly hasAny: (bookId: string) => boolean;
  readonly hasId: (id: string) => boolean;
  readonly list: (bookId: string, filter?: ListWritingResourcesFilter) => WritingResource[];
  readonly getById: (bookId: string, id: string) => WritingResource | null;
  readonly create: (input: CreateWritingResourceInput) => WritingResource;
  readonly update: (bookId: string, id: string, input: UpdateWritingResourceInput) => WritingResource | null;
  readonly softDelete: (bookId: string, id: string, deletedAt: number) => WritingResource | null;
  readonly getHistory: (bookId: string, id: string) => WritingResource[];
  readonly findAcceptedChapter: (bookId: string, chapterNumber: number) => WritingResource | null;
};

/**
 * Access the historical writing_resource table without migrating or rewriting it.
 * This repository remains book-scoped so Runtime callers cannot resolve a resource
 * from another trusted book binding merely by knowing its id.
 */
export function createWritingResourceRepository(storage: StorageDatabase): WritingResourceRepository {
  ...
  function getById(bookId: string, id: string): WritingResource | null {
    const row = sqlite.prepare(
      `SELECT * FROM "writing_resource" WHERE book_id = ? AND id = ?`,
    ).get(bookId, id) as WritingResourceRow | undefined;
  ...
  return {
    hasAny(bookId) {
      const row = sqlite.prepare(
        `SELECT 1 AS present FROM "writing_resource" WHERE book_id = ? LIMIT 1`,
      ).get(bookId) as { present: number } | undefined;
      return Boolean(row);
    },

    hasId(id) {
      const row = sqlite.prepare(
        `SELECT 1 AS present FROM "writing_resource" WHERE id = ? LIMIT 1`,
    ...
    list(bookId, filter = {}) {
      ...
      if (typeof filter.chapterNumber === "number") {
        clauses.push("chapter_number = ?");
        params.push(filter.chapterNumber);
      }
      const rows = sqlite.prepare(
        `SELECT * FROM "writing_resource" WHERE ${clauses.join(" AND ")} ORDER BY COALESCE(chapter_number, 999999), updated_at DESC`,
      ).all(...params) as WritingResourceRow[];
    ...
    create(input) {
      ...
      const created = getById(input.bookId, input.id);
      if (!created) throw new Error(`writing_resource create failed: ${input.id}`);
      return created;
    },

    update(bookId, id, input) {
      const current = getById(bookId, id);
      ...
      return getById(bookId, id);
    },

    softDelete(bookId, id, deletedAt) {
      return this.update(bookId, id, { deletedAt, updatedAt: deletedAt });
    },

    getHistory(bookId, id) {
      const chain: WritingResource[] = [];
      let current = getById(bookId, id);
      const seen = new Set<string>();
      while (current && !seen.has(current.id)) {
        chain.push(current);
        seen.add(current.id);
        current = current.parentId ? getById(bookId, current.parentId) : null;
      }
      return chain;
    },
    ...
    findAcceptedChapter(bookId, chapterNumber) {
      const row = sqlite.prepare(
        `SELECT * FROM "writing_resource" WHERE book_id = ? AND type = 'chapter' AND status = 'accepted' AND chapter_number = ? AND deleted_at IS NULL ORDER BY version DESC, updated_at DESC LIMIT 1`,
      ).get(bookId, chapterNumber) as WritingResourceRow | undefined;
      return row ? fromRow(row) : null;
    },
  };
}
