 ...
 * platform enum, and carries the writing-side expectations (chapter length,
 * hook density, AI-ratio tolerance, sensitive strictness) that the writing
 * chain can surface as warnings.
 */
...
export interface PlatformProfile {
  ...
  /** Estimated AI-ratio tolerance; 0 means the platform is effectively zero-tolerance. */
  readonly aiRatioTolerance: number;
  /** Whether block-severity sensitive hits should stop a formal save. */
  readonly blockOnSensitiveBlock: boolean;
...
const PROFILES: Record<SupportedPlatform, PlatformProfile> = {
  fanqie: {
    ...
    aiRatioTolerance: 0.2,
    blockOnSensitiveBlock: true,
    notes: [
