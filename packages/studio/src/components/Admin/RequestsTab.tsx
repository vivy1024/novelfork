...
export function RequestsTab({ onOpenRun }: RequestsTabProps) {
  ...
  const loadLogs = async () => {
    setLoading(true);
    try {
      const data = await fetchJson<{ logs: RequestLog[]; total: number; summary?: RequestSummary }>(`/api/admin/requests?limit=${limit}`);
      setLogs(data.logs);
              ...
              {runSummary.latestRuns.map((run) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-lg border p-3 text-left text-sm transition-colors ${isSelected ? "border-primary/40 bg-primary/5" : "border-border/60 bg-muted/30 hover:bg-muted/50"}`}
                  >
