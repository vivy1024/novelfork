...
const bootstrap = {
  ...
  books: [{ id: "book 1", title: "长夜", capabilities: { read: true, create: true } }],
  narrators: [{ id: "narrator 1", bookId: "book 1", title: "作者助手", capabilities: { read: true, send: true, interrupt: true } }],
...
describe("Runtime product contract", () => {
  it("maps bootstrap records and fails closed for omitted mutation capabilities", () => {
    const mapped = mapRuntimeBootstrap({
      ...
      narrators: [],
      model: {},
      capabilities: { books: { read: true }, narrators: { read: true }, workspace: { read: true } },
    });
    ...
    const omittedReads = mapRuntimeBootstrap({
      ...
      narrators: [{ id: "narrator-no-read", bookId: "book-no-read", title: "未授权叙述者" }],
      model: {},
  ...
  it("only calls bootstrap and book-scoped narrator/workspace paths", async () => {
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path === "/api/novelfork/books/book%201/status") return response(pendingOperation);
      if (path === "/api/novelfork/books/book%201/retry") return response({ ...pendingOperation, state: "ready" });
      if (path === "/api/novelfork/books/legacy%2Fbook/claim") return response({ ...pendingOperation, bookId: "legacy/book", state: "ready" });
      if (path === "/api/novelfork/books/book%201/repair") return response({ ...pendingOperation, state: "ready" });
      if (path.endsWith("/narrators") && !path.includes("narrator%201")) return response({ narrators: bootstrap.narrators });
      if (path.endsWith("/workspace")) return response({ book: bootstrap.books[0], resources: [], capabilities: { read: true, create: true, update: true } });
      if (path.endsWith("/workspace/chapters")) return response({ resource: { id: "chapter:1", kind: "chapter", title: "第 1 章", content: "", capabilities: { read: true, update: true } } });
      if (path.endsWith("/workspace/resources/chapter%3A1")) return response({ resource: { id: "chapter:1", kind: "chapter", title: "第 1 章", content: "正文", capabilities: { read: true, update: true } } });
      return response({});
    });
    const client = createRuntimeProductClient({ fetch: { token: "runtime-token", fetchImpl } });
    ...
    expect(paths.some((path) => path.startsWith("/api/sessions") || path.startsWith("/api/providers") || path.startsWith("/api/settings") || path.startsWith("/api/onboarding"))).toBe(false);
    expect(new Headers(fetchImpl.mock.calls[0]?.[1]?.headers).get("authorization")).toBe("Bearer runtime-token");
    expect(new Headers(fetchImpl.mock.calls[1]?.[1]?.headers).get("Idempotency-Key")).toBe("create-book-1");
    expect(fetchImpl.mock.calls[1]?.[1]?.body).toBe(JSON.stringify({ title: "长夜" }));
    expect(fetchImpl.mock.calls[8]?.[1]?.body).toBe(JSON.stringify({ title: "第 1 章" }));
    expect(fetchImpl.mock.calls[9]?.[1]?.body).toBe(JSON.stringify({ content: "正文" }));
  });
  ...
  it("uses only encoded book-scoped product paths and never sends projectId", async () => {
    ...
    const skillInput = { name: "新 skill", description: "作品技能", content: "正文", projectId: "forbidden" };
    const skillUpdate = { name: "重命名/skill", description: "更新", content: "新正文", projectId: "forbidden" };
    const hookInput = {
    ...
    const hookUpdate = { matcher: "Edit?", enabled: false, projectId: "forbidden" };
    ...
    expect(calls.map(([, init]) => init.method ?? "GET")).toEqual([
      "GET", "PUT", "GET", "GET", "POST", "PUT", "DELETE", "GET", "POST", "PUT", "DELETE",
    ]);
    expect(JSON.parse(String(calls[1]?.[1].body))).toEqual({ action: "disable" });
    expect(JSON.parse(String(calls[4]?.[1].body))).toEqual({ name: "新 skill", description: "作品技能", content: "正文" });
    expect(JSON.parse(String(calls[5]?.[1].body))).toEqual({ name: "重命名/skill", description: "更新", content: "新正文" });
    ...
    expect(JSON.parse(String(calls[9]?.[1].body))).toEqual({ matcher: "Edit?", enabled: false });

    const serializedRequests = calls.map(([path, init]) => `${path}\n${String(init.body ?? "")}`).join("\n");
  ...
  it("encodes both book and narrator identifiers in the book-scoped path", () => {
    expect(buildBookScopedNarratorPath("book/a", "narrator?1", "messages")).toBe("/api/books/book%2Fa/narrators/narrator%3F1/messages");
  });

  it("fails closed for an unknown contract version and malformed workspace DTOs", async () => {
    const mapped = mapRuntimeBootstrap({ ...bootstrap, contractVersion: "future", features: { runtimeNarratorParity: true } });
    expect(mapped.contractVersion).toBeNull();
    expect(Object.values(mapped.features).every((value) => value === false)).toBe(true);

    const client = createRuntimeProductClient({ fetch: {
      fetchImpl: vi.fn(async () => response({ book: bootstrap.books[0], resources: [{ id: "", kind: "chapter", title: "坏资源" }], capabilities: { read: true } })),
    } });
    await expect(client.getWorkspace("book 1")).resolves.toMatchObject({ resources: [] });
  });
