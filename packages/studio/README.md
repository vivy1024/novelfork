...
| `/next/books/:bookId` | 已挂载独立 Writing Workbench 壳，Workbench 资源树已接入 resource contract adapter 映射，并支持 canvas/viewer、保存、只读禁用、tool-result viewer 与 dirty canvasContext |
| `/next/settings` | 设置 |
| `/next/routines` | 套路 |
| `/next/search` | 搜索 |
