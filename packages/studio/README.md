...
| 路由 | 说明 |
|------|------|
| `/next` | 当前入口；Agent Shell 路由壳，主入口已切断旧三栏 WorkspacePage 默认依赖，失败三栏实验与旧 ChatWindow/轻量 `/api/chat` 源码已从 Studio typecheck 构建路径正式退役；v0.1.0 Release Readiness Task 7-8 已把主区从 `Agent Shell` 开发占位页升级为作者首页，复用 `useShellData` 展示最近作品、最近会话、模型健康、快速动作和空态；Task 13 主路径 E2E 已在 clean/isolated root 验活首页→作品→工作台→会话→设置/Provider/关于→Routines |
