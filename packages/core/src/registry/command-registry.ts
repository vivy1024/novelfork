...
export const RUNTIME_COMMAND_REGISTRY: readonly RuntimeCommandDefinition[] = [
  ...
  },
  {
    ...
    description: "按小说写作主链路生成下一章候选：cockpit.snapshot → pgi.ask → scene.spec → pipeline.write 为正式生成路径，candidate.create_chapter 仅保存已有正文。",
    scope: "novel",
    inputSchema: TEXT_ARGS,
    ...
    toolDependencies: ["cockpit.snapshot", "pgi.ask", "scene.spec", "pipeline.write", "candidate.create_chapter"],
