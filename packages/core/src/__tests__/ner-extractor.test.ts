...
import { extractEntities, Entity } from '../utils/ner-extractor';
...
describe('NER Extractor', () => {
  describe('人名识别', () => {
    it('应该识别常见姓氏的人名', () => {
      ...
      expect(persons.every(p => p.confidence === 0.9)).toBe(true);
    });
    ...
    it('应该识别修仙小说中的人名', () => {
      ...
      expect(persons.some(p => p.text === '叶凡')).toBe(true);
      expect(persons.some(p => p.text === '萧炎')).toBe(true);
      expect(persons.some(p => p.text === '林动')).toBe(true);
    });
    ...
    it('应该识别复姓人名', () => {
      ...
      expect(persons.some(p => p.text.includes('欧阳'))).toBe(true);
      expect(persons.some(p => p.text.includes('上官'))).toBe(true);
    });

    it('应该过滤掉以地名后缀结尾的假人名', () => {
      const text = '张城是一座古老的城市';
      const entities = extractEntities(text);
      const persons = entities.filter(e => e.type === 'person' && e.text === '张城');
  ...
  describe('地名识别', () => {
    it('应该识别常见地名后缀', () => {
      ...
      expect(locations.some(l => l.text === '青云宗')).toBe(true);
      expect(locations.some(l => l.text === '天元城')).toBe(true);
      expect(locations.some(l => l.text === '玄天界')).toBe(true);
    ...
    it('应该识别山川河流', () => {
      ...
      expect(locations.some(l => l.text === '昆仑山')).toBe(true);
      expect(locations.some(l => l.text === '黄河')).toBe(true);
    });
    ...
    it('应该识别宗门派系', () => {
      ...
      expect(locations.some(l => l.text === '天剑派')).toBe(true);
      expect(locations.some(l => l.text === '玄武宗')).toBe(true);
      expect(locations.some(l => l.text === '青莲教')).toBe(true);
    });
  });
  ...
  describe('术语识别', () => {
    it('应该识别功法术语', () => {
      ...
      expect(terms.some(t => t.text === '九阳神功')).toBe(true);
      expect(terms.some(t => t.text === '御剑术')).toBe(true);
    ...
    it('应该识别心法秘籍', () => {
      ...
      expect(terms.some(t => t.text === '天罡诀')).toBe(true);
      expect(terms.some(t => t.text === '上古真经')).toBe(true);
    });
    ...
    it('应该识别招式技能', () => {
      ...
      expect(terms.some(t => t.text.includes('掌'))).toBe(true);
      expect(terms.some(t => t.text.includes('剑'))).toBe(true);
    });
  });
  ...
  describe('道具识别', () => {
    it('应该识别神兵利器', () => {
      ...
      expect(items.some(i => i.text === '轩辕剑')).toBe(true);
      expect(items.some(i => i.text === '定海神珠')).toBe(true);
      expect(items.some(i => i.text === '炼妖壶')).toBe(true);
    ...
    it('应该识别丹药法宝', () => {
      ...
      expect(items.some(i => i.text === '筑基丹')).toBe(true);
      expect(items.some(i => i.text === '紫金铃')).toBe(true);
    });
    ...
    it('应该识别装备器物', () => {
      ...
      expect(items.some(i => i.text === '龙鳞甲')).toBe(true);
      expect(items.some(i => i.text === '青龙偃月刀')).toBe(true);
    });
  });
  ...
  describe('混合文本识别', () => {
    ...
    it('应该正确去重相同实体', () => {
      ...
      const personCount = entities.filter(e => e.text === '叶凡').length;
      const locationCount = entities.filter(e => e.text === '青云宗').length;
